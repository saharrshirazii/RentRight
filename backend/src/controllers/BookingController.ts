...
//READ - GET /bookings - get all bookings for current guest
...
//READ - GET /bookings/host - get all bookings for current host's properties
export const getHostBookings = async (req: Request , res: Response , next: NextFunction): Promise<void> => {
    try {
        ...
        const hostProperties = await Property.find({ owner: hostObjectId }).select('_id');
